...
evalkit_jshosted.setAttribute('src', 'https://emory.evaluationkit.com/CanvasScripts/emory.js?v=8');
...
function hide_settings() {
  ...
  document.querySelector("a[class$='reset_course_content_button']").style.display = "none";
  // hide reset course content button
  document.querySelector("a[href*='confirm_action?event=conclude']").style.display = "none";
  //hide conclude button
  ...
  try {
  /*we use try & catch for delete button because it doesn't always appear
  it would generate an error and break further modifications without it  */
    document.querySelector("a[href*='confirm_action?event=delete']").style.display = "none";
    //hide delete button by modifying CSS
  }
  catch (err) {console.log("no delete course link detected")}
}
